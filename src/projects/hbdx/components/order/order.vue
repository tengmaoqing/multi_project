<template>
  <div class="container order-view">
    <order-item
      ...
      @remarkOrder="showRemarkDialog"></order-item>
      <!-- 修改备注dialog -->
      <el-dialog
        class="remark-dialog"
        :title="$t('tip')"
        ...
        <div>{{ $t('orderDetail.fillRemark')}}：</div>
        <textarea class="content-input" v-model="remark"></textarea>
        <h-button class="h-btn btn-lg" size="large" @click="closeRemarkDialog">{{ $t('cancel') }}</h-button>
        <h-button class="h-btn btn-lg" size="large" type="primary" :disabled="!remark.length" @click="submitRemark">{{ $t('ensure') }}</h-button>
      </el-dialog>
      <!-- 取消订单dialog -->
      <el-dialog
        class="cancel-dialog"
...
import OrderItem from '@/core/order-item/index';
